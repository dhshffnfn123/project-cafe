...
public class OrderFrame extends DefaultFrame {
	...
	public OrderFrame() {
		...
		payment.setBounds(600, 600, 50, 50);
		...
		choosepage.setFont(new Font("맑은 고딕", Font.BOLD, 23));
		...
		for (int i = 0; i < 11; i++) {
			...
			drinks.setBackground(new Color(243, 243, 243));
		}
		...
		for (int i = 0; i <= 6; i++) {
			...
			products.setBackground(new Color(243, 243, 243));
		}
		// RTD 버튼들 추가 (타입 210)
		rtds.addTab("RTD", makeItemButtons("RTD", 210));
		rtds.setOpaque(true);
		rtds.setBackground(new Color(243, 243, 243));
		rtds.setFont(new Font("맑은 고딕", Font.BOLD, 13));
		// 옵션 버튼들 추가 (타입 120)
		option.addTab("옵션", makeItemButtons("option", 120));
		option.setOpaque(true);
		option.setBackground(new Color(243, 243, 243));
	...
	private ArrayList<MenuButton> itemButtons(String itemType, int typeNum) {
		...
		for (int i = 0; i < name.size(); i++) {
			...
			btns.get(i).setPreferredSize(new Dimension(183, 188));
			btns.get(i).setFont(new Font("맑은 고딕", Font.BOLD, 15));
			btns.get(i).setBackground(new Color(190, 190, 190));
	...
	private JPanel makeItemButtons(String itemType, int typeNum) {
		...
		for (int i = 0; i < btns.size() / 16 + 1; i++) {
			menuPanels.add(new JPanel(new FlowLayout(FlowLayout.LEFT, 0, 0))); // 그냥 FlowLayout 버튼 크기 맞춰서 맞춤
			menuPanels.get(i).setBackground(new Color(190, 190, 190));
		}
